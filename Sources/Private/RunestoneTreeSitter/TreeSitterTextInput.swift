...
package typealias TreeSitterReadCallback = (
    _ byteIndex: ByteCount,
    _ position: TreeSitterTextPoint
) -> TreeSitterByteRead?
...
package final class TreeSitterTextInput {
    ...
    package init(encoding: TSInputEncoding, callback: @escaping TreeSitterReadCallback) {
        self.encoding = encoding
        self.callback = callback
    }

    package func makeTSInput() -> TSInput {
        let payload = Unmanaged.passUnretained(self).toOpaque()
        return TSInput(payload: payload, read: read, encoding: encoding)
    }

    package func deallocate() {
