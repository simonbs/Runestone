...
#if DEBUG
private var previousUnsupportedPredicateNames: [String] = []
#endif

package final class TreeSitterTextPredicatesEvaluator<TreeSitterStringViewType: TreeSitterStringView> {
    private let match: TreeSitterQueryMatch
    private let stringView: TreeSitterStringViewType

    package init(match: TreeSitterQueryMatch, stringView: TreeSitterStringViewType) {
        self.match = match
        self.stringView = stringView
    }

    package func evaluatePredicates(in capture: TreeSitterCapture) -> Bool {
        ...
        for textPredicate in capture.textPredicates {
            ...
            case .unsupported(let parameters):
                #if DEBUG
                if !previousUnsupportedPredicateNames.contains(parameters.name) {
                    previousUnsupportedPredicateNames.append(parameters.name)
                    print(
                        "Unsupported predicate '\(parameters.name)'."
