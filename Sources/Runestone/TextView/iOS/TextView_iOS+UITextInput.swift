...
//        let locationNavigator = textViewController.locationNavigator
        switch direction {
        case .right:
//            let newLocation = locationNavigator.location(movingFrom: indexedPosition.index, byCharacterCount: offset, inDirection: .forward)
            return IndexedPosition(index: newLocation)
        case .left:
//            let newLocation = locationNavigator.location(movingFrom: indexedPosition.index, byCharacterCount: offset, inDirection: .backward)
            return IndexedPosition(index: newLocation)
        case .up:
//            let newLocation = locationNavigator.location(movingFrom: indexedPosition.index, byLineCount: offset, inDirection: .backward)
            return IndexedPosition(index: newLocation)
        case .down:
//            let newLocation = locationNavigator.location(movingFrom: indexedPosition.index, byLineCount: offset, inDirection: .forward)
            return IndexedPosition(index: newLocation)
        @unknown default:
            return nil
        }
    ...
    func closestPosition(to point: CGPoint) -> UITextPosition? {
        let index = textViewController.locationRaycaster.location(closestTo: point)
